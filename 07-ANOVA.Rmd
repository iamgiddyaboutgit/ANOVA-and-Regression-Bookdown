...
\[\text{SS}_{\text{total}} = \sum_{i=1}^{k} \sum_{j = 1}^{n_i} \left( y_{ij} - \bar{\bar{y}} \right)^2 = `r round(get_SS_total(fabricated_data$response), 3)`\]

\[\text{SS}_{\text{within}} = \sum_{i=1}^{k} \sum_{j = 1}^{n_i} \left(y_{ij} - \bar{y}_{i \cdot} \right)^2 = `r round(get_SS_within(fabricated_data$trt, fabricated_data$response), 3)`\]

\[\text{SS}_{\text{between}} = \sum_{i=1}^{k} n_i\left( \bar{y}_{i \cdot} - \bar{\bar{y}} \right)^2 = `r round(get_SS_between(fabricated_data$trt, fabricated_data$response), 3)`\]
