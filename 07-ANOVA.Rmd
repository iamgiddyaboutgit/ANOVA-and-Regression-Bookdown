...
The law of total variance, also known as EVE's law [@EVE][@EVE-intuition], is very important for understanding how ANOVA works.  The law says that for random variables \(X\) and \(Y\) we have
...
\begin{equation}
\label{eqn:EVE2}
\Var(Y) = \underbrace{\E[\Var(Y|G)]}_{\substack{\text{variance of } Y \\ \text{within groups}}} + \underbrace{\Var[\E(Y|G)]}_{\substack{\text{variance of } Y \\ \text{between groups}}}
\end{equation}
...
It is easy to see that both sides of \ref{eqn:EVE3} can be multiplied by \(N\) to give a relationship among sums of squares.  The resulting relationship is known as a partition of the sum of squares. [@partition_of_SS]
