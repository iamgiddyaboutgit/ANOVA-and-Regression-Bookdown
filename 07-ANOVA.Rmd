...
The law of total variance, also known as EVE's law [@EVE][@EVE-intuition], is very important for understanding how ANOVA works.  

\begin{equation}
\label{eqn:EVE}
\Var(Y) = \E[\Var(Y|\symbfup{X})] + \Var[\E(Y|\symbfup{X})]
\end{equation}
...
\begin{equation}
\label{eqn:EVE2}
\Var(Y) = \underbrace{\E[\Var(Y|\symbfup{X})]}_{\text{variance of Y within X}} + \underbrace{\Var[\E(Y|\symbfup{X})]}_{\text{variance of Y between X}}
\end{equation}

Recall [@variance] that the population variance of a finite population of size N is
...
where the sum is known as a sum of squares (SS).  It is easy to see that both sides of \ref{eqn:EVE2} can be multiplied by \(N\) to give a relationship among sums of squares.  The resulting relationship is known as a partition of the sum of squares. [@partition_of_SS]
